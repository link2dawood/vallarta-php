...
if(isset($_POST['submit'])){
   ...
   // Delivery Team login - check for users
   $select = mysqli_query($con, "SELECT * FROM `user_info` WHERE email = '$email' AND password = '$pass_hash'") or die('query failed');

   if(mysqli_num_rows($select) > 0){
      $row = mysqli_fetch_assoc($select);
      $_SESSION['user_id'] = $row['id'];
      $_SESSION['user_role'] = 'delivery';  // Mark as delivery team
      header('location:../order packaging details.php');
      exit;
   }else{
...
<head>
   ...
   <title>Delivery Team Login 420 Vallarta</title>
